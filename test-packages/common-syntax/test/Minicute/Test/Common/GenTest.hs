...
module Minicute.Test.Common.GenTest
  ...
  , hprop_identifier_gen_is_compatible_with_identifier_lexer
  ) where
...
import qualified Minicute.Test.Common.Gen as MCGen

hprop_integer_gen_is_compatible_with_integer_lexer :: Property
hprop_integer_gen_is_compatible_with_integer_lexer = do
  property $ do
    i <- forAll MCGen.integer
    i' <- evalEither $ parse (L.integer :: Parser Integer) "" (show i)
    i === i'

hprop_identifier_gen_is_compatible_with_identifier_lexer :: Property
hprop_identifier_gen_is_compatible_with_identifier_lexer = do
  property $ do
    ident <- forAll MCGen.identifier
